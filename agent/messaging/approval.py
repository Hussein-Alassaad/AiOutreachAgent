...
# How long a message can sit "awaiting" before it's flagged for a reminder.
# No dashboard-configurable setting exists for this yet (a Phase 9 decision)
# -- this is a sensible fixed default in the meantime.
...
def hold_message(message_id: str) -> dict:
    """
    Hold one message -- keeps it (and its lead) blocked from sending
    indefinitely until Mohamad comes back and approves or edits it. No
    reason field exists on `messages` to record why; that's a Phase 9
    dashboard decision, not this backend's to invent.
    """
    return repo.update_message(message_id, {"approval_status": "held"})


def messages_needing_reminder() -> list[dict]:
    """
    Messages still sitting "awaiting" longer than REMINDER_AFTER_HOURS -- the
    trigger Phase 8's actual WhatsApp reminder notification reads from. This
    function only identifies them; it does not send anything itself, since
    the notification-sending machinery doesn't exist until Phase 8.
    """
    cutoff = dt.datetime.now(dt.timezone.utc) - dt.timedelta(hours=REMINDER_AFTER_HOURS)
